...
final class DropDatabaseCommandTest
{
    ...
    public function setUpTest() : void
    {
        ...
        \HH\Asio\join($this->conn->rawQuery(sprintf("DROP DATABASE IF EXISTS %s", $dbname)));
        \HH\Asio\join($this->conn->rawQuery(sprintf("CREATE DATABASE %s", $dbname)));
    }
    ...
    public function dropDatabase(Assert $assert): void
    {
        ...
        $currentDatabases = $result->rows()->filter(($row) ==> $row->at('Database') === static::DB_NAME);
